...
/**
 * Integration tests for `eventDetailFromForm` focused on the "user" (member)
 * role. The controller is wired to a real EventService and a real in-memory
 ...
 * up an HTTP server.
 */
describe("EventController.eventDetailFromForm — user role (integration)", () => {
  ...
  it("renders events/detail with the event for an authenticated member viewing a published event", async () => {
    ...
    // Only HTMX requests should render without the base layout.
