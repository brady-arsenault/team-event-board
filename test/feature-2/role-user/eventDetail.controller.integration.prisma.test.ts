...
/**
 * Prisma-backed mirror of `eventDetail.controller.integration.test.ts` for
 * the "user" (member) role. Identical assertions; the repository is the real
 * Sprint 3 PrismaEventRepository against a fresh SQLite database.
 */
describe("EventController.eventDetailFromForm — user role (integration, Prisma)", () => {
  let db: PrismaTestDb;
  ...
  function signInAsMember(store: AppSessionStore): void {
  ...
  function build() {
    const repo = CreatePrismaEventRepository(db.prisma);
  ...
  it("renders events/detail with the event for an authenticated member viewing a published event", async () => {
    const { repo, controller } = build();
    const event = await repo.create(
      makeEvent({ status: "published", organizerId: USER_IDS.staff }),
    );
    const store = makeSessionStore();
    signInAsMember(store);
  ...
  it("responds 403 when a non-organizer member requests a draft", async () => {
    const { repo, controller } = build();
    const event = await repo.create(
  ...
  it("delegates to the service and passes through the acting member", async () => {
    const repo = CreatePrismaEventRepository(db.prisma);
    const service = CreateEventService(repo, silentLogger());
