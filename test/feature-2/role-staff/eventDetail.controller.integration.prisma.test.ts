 ...
 * the "staff" role. Identical assertions, real PrismaEventRepository.
 */
describe("EventController.eventDetailFromForm — staff role (integration, Prisma)", () => {
  const OTHER_ORGANIZER = "some-other-user";
  let db: PrismaTestDb;

  beforeAll(async () => {
    db = await setupPrismaTestDb();
    // The "non-organizer" foreign key referenced by the original tests.
  ...
  it("renders events/detail for a staff viewer on a published event they did not organize", async () => {
    ...
    const event = await repo.create(
      makeEvent({ status: "published", organizerId: OTHER_ORGANIZER }),
    );
  ...
  describe("draft visibility", () => {
    ...
    it("responds 403 when a staff viewer who is not the organizer requests a draft", async () => {
      const { repo, controller } = build();
      const event = await repo.create(
        makeEvent({ status: "draft", organizerId: OTHER_ORGANIZER }),
      );
      const store = makeSessionStore();
      signInAsStaff(store);
